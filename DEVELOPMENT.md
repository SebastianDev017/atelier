...
- **Demo stores (one per preset, password `1234`), one git branch each:** `contour-ho5kbo9x` → branch `main` (theme `contour/main` #159799967844) · `annex-r0zzjfjf` → branch `annex` (theme `atelier/annex` #192380338468) · `maison-10oym9w0` → branch `maison` (theme `atelier/maison` #189395697969). `listings/<preset>/listing.json` points at each store. A branch carries the full codebase plus THAT store's own `config/settings_data.json` `current` and its own `shopify://shop_images/...` picks (homepage hero + editorial, About studio image). Until 2026-09-13 all three themes were connected to `main`, so their editor saves overwrote each other's theme style in one shared `current`; Annex/Maison were reconnected to their own branches in Shopify Admin on 2026-09-13 (a reconnect creates a new theme ID; the old `annex/main` #192368509220 and `maison/main` #189394649393 were deleted).
- **Free shipping:** each store's REAL rate (Settings → Shipping and delivery: Standard $8, free from $150; Express $15) is store configuration that git never sees. The theme's claim is `cart_free_shipping_threshold` in each branch's `current`. Keep the two equal on all three stores (both are $150 since 2026-09-13). The schema default and all presets are 150 too, so a theme-style switch cannot drift it.
...
- The packaged ZIP is named from `theme_name` → `Contour-3.8.0.zip`.
